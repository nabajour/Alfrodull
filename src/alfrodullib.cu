...
__host__ bool prepare_compute_flux(
    ...
    if (interp_and_calc_flux_step) {
        ...
        if (!iso) {
            ...
            meanmolmass_interpol<<<mmmi_grid, mmmi_block>>>(
                dev_T_int,                              // in
                *(Alf_ptr->opacities.dev_temperatures), // in
...
void compute_radiative_transfer(
    ...
    double albedo    = Alf_ptr->albedo;

    double* cloud_opac_lay       = Alf_ptr->cloud_opac_lay;
    double* cloud_opac_int       = Alf_ptr->cloud_opac_int;
    double* cloud_scat_cross_lay = Alf_ptr->cloud_scat_cross_lay;
    double* cloud_scat_cross_int = Alf_ptr->cloud_scat_cross_int;
    double* g_0_tot_lay          = Alf_ptr->g_0_tot_lay;
    double* g_0_tot_int          = Alf_ptr->g_0_tot_int;

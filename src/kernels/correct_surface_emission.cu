...
__global__ void correct_surface_emission(double* delta_lambda,   // in
                                         double* planckband_lay, // in/out
                                         double  surf_albedo,
                                         double  T_surf,
                                         int     nwave,
                                         int     numlayers) {
    ...
    if (x < nwave) {
        ...
        for (int xl = 0; xl < nwave; xl++) {
            ...
            uncorr_emission +=
                delta_lambda[xl] * PI * planckband_lay[(numlayers + 1) + xl * (numlayers + 2)];
        }
